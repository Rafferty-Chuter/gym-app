...
export function buildProgrammeFromLLMPlan(
  ...
  assertLegacyWorkoutPathsAllowed(
    "buildProgrammeFromLLMPlan (structure LLM exercise ids + slot-filling prescriptions)"
  );
  const issues: string[] = [];
  ...
  const days = params.plan.days.map((d, idx) => {
    ...
    return {
      ...
      debugDayGenerator: "programme_structure_llm_path",
  ...
  return {
    programme: {
      ...
      notes: `DEBUG: generated by programme_structure_llm_path. Hybrid v2 generation. ${params.plan.briefRationale || "Validated against catalog constraints."}`,
      debugProgrammeGenerator: "programme_structure_llm_path",
